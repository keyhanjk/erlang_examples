...
-export([child/0, grandparent/0, parent/0, run/0]).
...
parent() ->
    ...
    io:format("I (parent) have a linked child: ~p~n",
              [Pid]),
    lists:foreach(fun (_) ->
                          P = spawn_link(links, child, []),
                          io:format("I (parent) have another linked child: "
                                    "~p~n",
                                    [P])
                  end,
                  lists:seq(1, 4)),
    ...
    receive
        {'EXIT', Pid, Reason} ->
            timer:sleep(10),
            io:format("I (parent) have a dying child(~p). Reason: "
                      "~p~n",
                      [Pid, Reason]),
...
grandparent() ->
    ...
    io:format("I (grandparent) have a linked child: "
              "~p~n",
              [Pid]),
    process_flag(trap_exit, true),
    receive
        {'EXIT', Pid, Reason} ->
            timer:sleep(10),
            io:format("I (grandparent) have a dead child(~p). "
                      "Reason: ~p~n",
                      [Pid, Reason]),
